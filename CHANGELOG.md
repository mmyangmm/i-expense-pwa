...
- 換匯每個幣別列新增 `C` 清除鍵，放在上移、下移、刪除按鈕右側，可清除該列目前暫存金額。
- `C` 清除後會將該幣別設為基準並清空基準金額，其他幣別即時歸零，避免殘留上一筆換算結果。
- 固定換匯列工具按鈕位置，讓四顆按鈕在手機寬度維持整齊排列。
- 將 PWA 資源快取版本升為 v17，並修正 service worker 核心資源版本，避免舊版 JS/CSS 被快取留住。
